...
func main() {
	...
	coordConfig, err := configLoader.LoadCoordinatorConfig(ctx, "plugin-coordinator")
	if err != nil {
	...
	pluginDir, err := coordConfig.GetString("plugin_dir", "/var/lib/bigskies/plugins")
	if err != nil {
		logger.Fatal("Failed to parse plugin_dir", zap.Error(err))
	}
	scanInterval, err := coordConfig.GetDuration("scan_interval", 300*time.Second)
	if err != nil {
	...
	cfg := &coordinators.PluginCoordinatorConfig{
		BrokerURL:    fullBrokerURL,
		PluginDir:    pluginDir,
		ScanInterval: scanInterval,
	}
	cfg.Name = "plugin-coordinator"
	cfg.LogLevel = *logLevel

	// Create plugin coordinator
	coordinator, err := coordinators.NewPluginCoordinator(cfg, logger)
	if err != nil {
